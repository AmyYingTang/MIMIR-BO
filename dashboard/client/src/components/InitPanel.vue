  ...
  <div class="init-layout">
    <div class="panel">
      ...
      <div v-if="step === 1" class="step-content">
        ...
        <DirField icon="📁" :label="t.projectDir" :hint="t.projectDirHint"
          :value="config.paths.project" :disabled="isReadonly" @pick="!isReadonly && openPicker('project')" />
        <div class="form-group">
          <label>📂 {{ t.workspaceDir }}</label>
          <div class="workspace-row">
            <input :value="config.paths.workspace" class="form-input mono" readonly />
            <button v-if="!isReadonly" class="btn btn-secondary btn-sm" @click="openPicker('workspace')">{{ t.selectOther }}</button>
          </div>
          <span class="form-hint">{{ isReadonly ? '' : (workspaceOverridden ? t.customized : t.autoGenerated) }}</span>
        </div>
      </div>
...
const config = reactive({
  ...
  paths: { mimir: '', tools: '', workspace: '', project: '' },
  user_content_dirs: [],
  source_docs: {},
})

const workspaceOverridden = ref(false)
...
async function openPicker(target) {
  const titles = {
    ...
    project: t.value.projectDir, workspace: t.value.workspaceDir,
  }
  try {
    ...
    if (target === 'tools' && !workspaceOverridden.value) updateAutoWorkspace()
    if (target === 'workspace') workspaceOverridden.value = true
  } catch (e) { /* cancelled */ }
}
...
function updateAutoWorkspace() {
  if (!config.paths.tools) return
  const slug = config.instance_name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '') || 'project'
  config.paths.workspace = config.paths.tools + '/.workspace/' + slug
}

const canNext = computed(() => {
  if (step.value === 0) return config.instance_name.trim() !== '' && config.project_type
  if (step.value === 1) return config.paths.mimir && config.paths.tools && config.paths.project
  return true
})

async function loadStepData(targetStep) {
  if (targetStep === 2 && !mimirScan.value && config.paths.mimir) {
    try { mimirScan.value = await api.scanMimirContent(config.paths.mimir, config.language) } catch (e) {}
  }
}

async function nextStep() {
  if (step.value === 0 && !isReadonly.value && !workspaceOverridden.value) updateAutoWorkspace()
  if (step.value === 1) {
...
.workspace-row .form-input { flex: 1; }
