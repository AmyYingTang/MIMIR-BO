...
Agent 的产出严格分为三层，确保 generic 模板可跨项目复用：
...
│   ├── ops-manual.yml        — Ops Manual 骨架模板
│   └── common-rules.md       — 通用写作指令模板
└── doc-gen.sh                — Runner 脚本

Layer 2: Project-specific（Step 1 + Step 2 动态生成，存到 output_dir）
├── analysis-report.md        — Step 1 扫描源文档产出
├── fill-order.md             — 填充依赖图
├── prompts/
│   ├── 00-common-rules.md    — 通用指令（从模板实例化）
│   ├── 01-limits.md          — 各页提取 prompt（含项目特定信息）
│   └── ...
├── mkdocs.yml
└── docs/*.md                 — 骨架 .md 文件（含 TODO 标记）
...
| 维度 | Layer 1 (Generic) | Layer 2 (Project-specific) |
|------|-------------------|---------------------------|
| 包含 | 页面结构、通用提取逻辑、术语规则 | 具体章节号、角色名、端点列表、字段名 |
| 不包含 | 任何项目特定内容 | — |
| 生命周期 | 随 agent 版本更新 | 每次运行时重新生成 |
| 示例 extract_strategy | "提取所有角色定义和权限分配" | "从 prd §3.1 提取 SuperAdmin/Admin/User 三种角色" |

### 第2批页面的动态生成

骨架模板中第2批（操作指南页）只提供**通用页面模式**（如 `guides/user-management.md`、`guides/content-*.md`），不列出具体页面。Step 2 (skeleton) 阶段，agent 结合 analysis-report.md 中发现的项目实际功能模块，动态决定具体需要哪些页面：

- 可增加模板中未列出的页面（项目有特殊功能）
- 可跳过项目不涉及的页面（如项目无 GPU 则跳过 gpu-setup.md）
- 为每个确定的页面生成 project-specific 的提取 prompt
...
骨架模板（`templates/{doc_type}.yml`）是 **generic** 的，不含项目特定内容。每种 doc_type 定义三类页面：

| 批次 | 内容 | 模板定义方式 |
|------|------|-------------|
| 第1批：参考数据页 | 权限矩阵、状态速查、配额限制等 | 模板中明确列出，通用性强 |
| 第2批：操作指南页 | 各功能模块的操作指南 | 模板只提供**通用页面模式**，具体页面由 Step 2 动态生成 |
| 第3批：综合页 | 快速上手、FAQ、首页概览 | 模板中明确列出，通用性强 |

各 doc_type 的页面差异：

| 维度 | User Guide | Admin Guide | Ops Manual |
|------|-----------|-------------|------------|
| 第1批 | 配额限制、权限说明、状态速查 | 完整权限矩阵、API 参考、状态速查、错误码 | 端口服务、环境变量、目录结构、安全清单 |
| 第2批 | 按用户操作流程动态生成 | 按管理工作流动态生成 | 按运维生命周期动态生成 |
| 第3批 | 快速上手、FAQ、首页概览 | 管理员快速上手、FAQ、首页概览 | 故障排查、快速上手、首页概览 |
| 术语级别 | 禁止技术术语 | 允许管理概念 | 允许完整技术细节 |

详见 `templates/` 下各 `.yml` 文件。

---

## 5. Agent 主 Prompt（System Prompt）
...
1. 读取对应的 generic 骨架模板（templates/{doc_type}.yml）
2. 读取 analysis-report.md 获取项目特定的源文档结构信息
3. 动态确定第2批（操作指南）页面列表：
   - 模板只提供通用页面模式（如 guides/user-management.md）
   - 根据 analyze report 中的实际功能模块，确定具体需要哪些页面
   - 可增加模板中未列出的页面，可跳过项目不涉及的页面
4. 为每个页面生成 project-specific 的提取 prompt：
   - 通用提取逻辑来自 generic 模板的 extract_strategy
   - 项目特定信息（章节号、角色名、端点列表等）来自 analyze report
   - 保存到 {output_dir}/prompts/ 目录
5. 为每个页面生成带 TODO 标记的 .md 骨架文件
6. 生成 mkdocs.yml 配置文件
7. 生成 fill-order.md（三批执行顺序）
```

**关键：Step 2 是三层分离的桥梁。** generic 模板说"提取所有角色定义和权限分配"，analyze report 说"prd §3.1 定义了 SuperAdmin/Admin/User 三种角色"，Step 2 将两者结合，生成 prompt："从 prd §3.1 提取 SuperAdmin/Admin/User 三种角色定义和创建方式"。
